...
public class Collision {
    ...
    public static boolean hasCollision(Movable m1, Movable m2) {
        double distance = Vec2D.distance(m1.getNextPos(), m2.getNextPos()) - m1.getRadius() - m2.getRadius();
        return distance <= 0;
    }

    public static boolean hasCollision(Movable m1, Stationary s1) {
        double distance = Vec2D.distance(m1.getNextPos(), s1.getPosition()) - m1.getRadius() - s1.getRadius();
        return distance <= 0;
    }

    public static Vec2D getContactVector(Movable m1, Movable m2) {
        Vec2D contact = m1.getPosition().subtract(m2.getPosition());
        contact.normalize(1);
        return contact;
    }

    public static Vec2D getContactVector(Movable m1, Stationary s1) {
    ...
    public void setData(Movable m1, Vec2D contactVector) {
        this.mContactVector = contactVector;
    ...
    private Vec2D getNewStatVec(Movable m, double magnitude) {
